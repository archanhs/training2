    ...
    <table class="table table-hover" id="myTable">
        <thead>
            <tr>
                ...
                <th scope="col">Sifat Dasar</th>
                <th scope="col">Action</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($sifatDasar as $key => $value) { ?>
                <tr>
                    <th scope="row"><?= $key + 1 ?></th>
                    <td><?= $value->kode ?></td>
                    <td><?= $value->sifat_dasar ?></td>
                    <td>
                        <div class="d-flex">
                            ...
                            <form action="<?= base_url() ?>form2/deletesifatdasar" method="POST">
                                <input type="hidden" name="id" value="<?= $value->id ?>">
                                <input type="hidden" name="name" value="<?= $value->sifat_dasar ?>">
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?= base_url('form2/addsifatdasar') ?>" method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="exampleInputPassword1" class="form-label">Sifat Dasar</label>
                        <input type="text" class="form-control" name="sifatDasar" required>
                    </div>
                </div>
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?= base_url('form2/updatesifatdasar') ?>" method="POST" id="formInput">
...
<script>
    $(".btn-edit").on("click", function() {
        ...
        $.ajax('<?= base_url('form2/getsifatdasarbyid') ?>', {
            ...
            success: function(data, status, xhr) {
                var html = `<div class="mb-3">
                        ...
                        <input type="hidden" name="old" value="${data.sifat_dasar}" >
					</div>
					<div class="mb-3">
						<label for="exampleInputPassword1" class="form-label">Jenis Kelamin</label>
						<input type="text" class="form-control" name="sifatDasar" value="${data.sifat_dasar}" required>
					</div>`;
